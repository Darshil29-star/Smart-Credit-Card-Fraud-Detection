...
def run_pipeline(data_path, model_dir):
    ...
    precision_pr, recall_pr, _ = precision_recall_curve(y_test, test_cal_probs)
    pr_auc_score = auc(recall_pr, precision_pr)
    precision_val = precision_score(y_test, test_preds)
    recall_val = recall_score(y_test, test_preds)
    f1_val = f1_score(y_test, test_preds)
    
    print(f"\n[*] PR-AUC Baseline Score: {pr_auc_score:.4f}")
...
| :--- | :--- | :--- |
| **PR-AUC** | `{pr_auc_score:.4f}` | **Area Under the Precision-Recall Curve** (Primary metric for highly imbalanced fraud datasets). |
| **Precision** | `{precision_val:.4f}` | Proportion of predicted frauds that were actually fraudulent. |
| **Recall** | `{recall_val:.4f}` | Proportion of actual fraudulent transactions successfully identified. |
| **F1-Score** | `{f1_val:.4f}` | The harmonic mean of Precision and Recall. |

## Technical Note: Class Imbalance Handling
During training, the LightGBM classifier leveraged a dynamic `scale_pos_weight` multiplier of **{scale_weight:.2f}** to shift the decision boundary and prioritize capturing complex fraud behaviors.
"""
    with open("RESULTS.md", "w") as f:
        f.write(md_content)
    print("[+] RESULTS.md successfully generated with exact local metrics.")
